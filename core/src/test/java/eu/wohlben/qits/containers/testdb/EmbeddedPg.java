 ...
 *
 * <p><b>Why not a container.</b> A clone of this repo alone builds and tests green — no monorepo,
 * no credentials and <em>no docker</em> — and that rule bites hardest here, because docker is this
 * repository's subject: a suite that needed a daemon to say what the orchestrator does could never
 * say it without one. Testcontainers and Quarkus dev services are therefore both out, while the
 * lineages still have to run against the engine they ship on. Zonky resolves real postgres binaries
 * as ordinary Maven artifacts and this class spawns them as a child process: a dependency, not a
 * daemon.
 *
 ...
 *
 * <p>A copy of the class every sibling repo carries, because a test class is not a dependency
 * anything else can declare. What must never be shared is the DATABASE NAME: every (module,
 * datasource) pair names its own, so two surefire JVMs on one host cannot mean one database. See
 * {@link EmbeddedPgConfigSource}.
 */
